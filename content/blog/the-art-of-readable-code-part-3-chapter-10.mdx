...
Lời khuyên trong chương này là luôn tích cực xác định các bài toán con không liên quan và tách biệt nó, cụ thể là:

1. Xem một function hoặc một block cụ thể và tự đặt câu hỏi cho mình rằng: “Mục tiêu chính của đoạn code này là gì?”
...
Việc tách code thành các hàm con là một phần trong quá trình làm việc hàng ngày, trong đó, hàm con này chỉ cần thực hiện nhiệm vụ cụ thể mà không cần phải quan tâm đến ngữ cảnh hoặc mục tiêu lớn hơn của chương trình.
...
Hầu hết phần code bên trong vòng lặp hiện đang xử lý một vấn đề con không liên quan, đó là tính toán khoảng cách giữa hai điểm dựa trên vĩ độ và kinh độ. Vì vậy hãy tách nhỏ đoạn xử lý này thành một hàm riêng biệt có tên là `spherical_distance()`
...
<CodePlayground
  files={{
    "index.js": `var format_pretty = function (obj, indent) {
  // Handle null, undefined, strings, and non-objects.
  if (obj === null) return "null"
  if (obj === undefined) return "undefined"
  if (typeof obj === "string") return '"' + obj + '"'
...
Rõ ràng chúng ta có thể tách riêng nó thành một hàm `get_cookie()` để thực hiện việc này:
...
Một cách tốt hơn để đặt cookie sẽ là wrap đoạn code trên vào trong 1 function và truyền vào những đối số nhìn thân thiện hơn như sau:
...
Vấn đề xảy ra ở đây là `Cipher` đang nhận vào khá nhiều đối số cồng kềnh. Ở đây mục đích mà chúng ta đang cần làm là "Hãy mã hoá thông tin người dùng vào URL" nhưng phần lớn của đoạn code trên chỉ là thực hiện mã hoá đối tượng thành một chuỗi URL thân thiện với người dùng", hãy tách riêng những logic phụ đó thành một hàm khác, như sau:
